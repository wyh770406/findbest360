...
class GroupbuySubcate
  ...
  def self.get_same_cate(cate, subcate)
    p_cate = GroupbuyCate.where({:name => cate}).first
    s_cate = GroupbuySubcate.where({:name => subcate}).first
    if s_cate.present?
      return s_cate, s_cate.groupbuy_cate
    else
      if p_cate.present?
        sa = self.get_child_cate(p_cate, subcate)
        return sa, p_cate
      else
