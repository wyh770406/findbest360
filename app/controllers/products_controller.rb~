class ProductsController < ApplicationController
  ...
  def index
    ...
    @search_key = @end_product.name

    con = {:price.ne=>nil,:price.gt=>0}
    ...
    logger.info("ssssssssssssssssssssss#{con}4444444444444444444444444444444444")
    ...
    merchant_sort_ids = Product.includes(:end_products,:brands,:brand_types,:merchants).where(con.merge!({:end_product_id=>@end_product.id})).distinct(:merchant_id)
    end_product_sort_ids = Product.includes(:end_products,:brands,:brand_types,:merchants).where(con.merge!({:end_product_id=>@end_product.id})).distinct(:end_product_id)
    brand_sort_ids = Product.includes(:end_products,:brands,:brand_types,:merchants).where(con.merge!({:end_product_id=>@end_product.id})).distinct(:brand_id)
    #Rails.logger.debug { "merchant_sort_products:#{@merchant_sort_products}" }
    brand_type_sort_ids = Product.includes(:end_products,:brands,:brand_types,:merchants).where(con.merge!({:end_product_id=>@end_product.id})).distinct(:brand_type_id)
    ...
    @products = Product.includes(:end_products,:brands,:brand_types,:merchants).where(con.merge!({:end_product_id=>@end_product.id,:brand_type_id => nil})).order_by(order_con).page(params[:page]).per(40)
  ...
  def search
    ...
    ids = []
    ids = Search.query(params[:search_key].strip,:limit=>5000)
    #result.each do |prod|
  ...
  def detail
    ...
    @brand_type_products = @brand_type.products.where({:price.ne=>nil,:price.gt=>0}).asc("price")
    @end_product = @brand_type_products.first.end_product
    
    price_arr = @brand_type_products.map(&:price)
    average_price = price_arr.sum / price_arr.size
    ...
    total_score_count = 0
    @total_comments = []
    for brand_type_product in @brand_type_products
      if brand_type_product.kind=="jingdong"
        @jingdong_product = brand_type_product
      end

      if brand_type_product.score
        total_comments_score = total_comments_score + brand_type_product.score
        total_score_count = total_score_count + 1
      end
      if brand_type_product.comments
        @total_comments = @total_comments + brand_type_product.comments
      end
      @total_comments_count = @total_comments_count + brand_type_product.comments.count
    end

    if total_score_count > 0
      @average_comments_score = (total_comments_score.to_f / total_score_count).round(2)
    end

    if @end_product
      @similar_products = (@end_product.products.where({:price.gt => average_price*0.8, :price.lt => average_price*1.2 } ) - @brand_type_products).uniq.first(10)
    end
  ...
  def sort_end_product(ids)
    ...
    all_end_products = EndProduct.any_in(:_id =>ids).asc("order_num")
    got_end_products = []
    end_products = middle_products.map do |middle_product|

      all_end_products = all_end_products - got_end_products
      got_end_products = get_end_product(all_end_products,middle_product.id)
      {
        :middle_product => middle_product,
        :end_product=> got_end_products#EndProduct.any_in(:_id =>ids).where(:middle_product_id=>middle_product.id).asc("order_num")
      }
      #all_end_products = all_end_products - got_end_products
  ...
  def get_end_product(allend_products,vmiddle_product_id)

    arr_end_products = []
    allend_products.each do |end_product|
      if end_product.middle_product_id == vmiddle_product_id 
         arr_end_products << end_product 
      end
    end
    arr_end_products
